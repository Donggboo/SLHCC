...
namespace ns3
{
	...
	Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp)
	{
		...
		if (m_cc_mode == 11)
		{
			if (qp->snd_cnt == 0 && qp->m_size > 25000)
			{
				ih.rate = qp->m_rate / DataRate(50000000);
				if (Simulator::Now().GetTimeStep() < qp->q_t)
				{
					ih.u = 253;
				}
				else
...
// printf("%.6lf\n", q->mlx.m_alpha);
	...
	void RdmaHw::HandleLHCC(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch)
	{
		...
		{
			uint64_t tau = ch.nt.ih.GetTimeDelta(qp->ip_hop[ch.sip]);
			double qlen = ch.nt.ih.GetQlen();
			double duration = tau * 1e-9;
			...
			if (qlen > 0)
			{
				double t = qlen * 8 * 1e9 / txRate;
				if (t > 2 * T)
				{
					qp->q_t = std::max(qp->q_t, (u_int64_t)(Simulator::Now().GetTimeStep() + t - 2 * T));
				}
			}
			double rtt_q = 0;
			qp->ip_hop[ch.sip] = ch.nt.ih;
			...
			{
				rtt_q += (double)it.second.GetQlen() * 8 * 1e9 / (double)it.second.GetLineRate();
			}
			...
			qp->ip_to_u[ch.sip] = u * tau / T + qp->ip_to_u[ch.sip] * (T - tau) / T;
			double U = 0;
			...
			if (U > qp->ip_to_u[ch.sip])
				return;
			DataRate new_rate = m_sp / U * m_targetUtil + m_rai;
			if (new_rate > qp->m_max_rate)
			{
				new_rate = qp->m_max_rate;
			}
